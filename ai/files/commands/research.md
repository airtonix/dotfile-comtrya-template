...
This command is ideal for architecture decision documentation, technology evaluation, best practices investigation, and team training materials.
...
3. **Storage Path** (optional) - If provided, save outputs there. If omitted, use the current working directory.
...
2. **Validate request structure:**
   ...
   - [ ] Storage path (optional) acknowledged
...
5. **Delegate subtopics to parallel subagents:**
   ...
   task(
     ...
     prompt: "Research subtopic: [Subtopic 1]
              ...
              Avoid: [ThingsToAvoid]
              Storage path: [StoragePath or current working directory]

              CRITICAL: Load any applicable skills to enhance your research methodology.
...
6. **Validate subagent outputs:**
   - [ ] Each subtopic produced research output files with required naming prefix
...
- Produce research outputs as single files using prefix: research-{hash}-{parent_topic}-{child_topic}.md
...
4. **Unified Storage:** Use the requested storage path (or current working directory); outputs are single files with prefix `research-{hash}-{parent_topic}-{child_topic}.md`.
...
3. Use the required filename prefix: research-{hash}-{parent_topic}-{child_topic}.md
4. Save output to the requested storage path (or current working directory)
...
OUTPUT:
- Produce research outputs in the selected storage location
...
- ✅ Organized output → research files that are easy to re-read and cite
